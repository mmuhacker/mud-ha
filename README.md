...
# 🌐 HTTP Headers Analyzer     أداة تحليل ترويسات HTTP
...
- [الوصف](#الوصف)
- [المميزات](#المميزات)
- [التثبيت](https://github.com/mmuhacker/mud-ha/blob/main/README.md#%EF%B8%8F-%D8%A7%D9%84%D8%AA%D8%AB%D8%A8%D9%8A%D8%AA)
...
أداة لتحليل ترويسات HTTP لأي موقع وتقييم مستوى أمانه، مكتوبة بلغة Python وتعمل على Termux و Kali Linux. تكشف الترويسات الأمنية المفقودة وتعرض تقييماً شاملاً لأمان الموقع.
...
- 🌐 عرض جميع ترويسات HTTP للموقع
- 🛡️ تحليل ترويسات الأمان (HSTS, CSP, X-Frame-Options...)
- ⚠️ كشف معلومات الخادم الحساسة (Server, X-Powered-By)
- 📊 تقييم أمان الموقع (ممتاز / متوسط / ضعيف)
- 🔄 مقارنة HTTP مقابل HTTPS
- 🌍 واجهة عربية بالكامل
...
```bash
curl -o $PREFIX/bin/mud_ha.py https://raw.githubusercontent.com/mmuhacker/mud-ha/main/mud_ha.py
```

**الخطوة 5 — إعطاء صلاحية التشغيل**
```bash
chmod +x $PREFIX/bin/mud_ha.py
```

**الخطوة 6 — إنشاء رابط الاختصار**
```bash
ln -sf $PREFIX/bin/mud_ha.py $PREFIX/bin/ha
```
...
```bash
sudo curl -o /usr/local/bin/mud_ha.py https://raw.githubusercontent.com/mmuhacker/mud-ha/main/mud_ha.py
```

**الخطوة 4 — إعطاء صلاحية التشغيل**
```bash
sudo chmod +x /usr/local/bin/mud_ha.py
```

**الخطوة 5 — إنشاء رابط الاختصار**
```bash
sudo ln -sf /usr/local/bin/mud_ha.py /usr/local/bin/ha
```
...
| الوضع | الوصف |
|-------|-------|
| `1` تحليل موقع | عرض وتحليل كل ترويسات HTTP |
...
| curl | تنزيل الأداة |
| اتصال بالإنترنت | للاتصال بالمواقع |
